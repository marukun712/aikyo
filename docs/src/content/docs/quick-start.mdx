...
<Aside type="caution">生成された`package.json`には必ず`"type": "module"`を設定してください。ES Modulesとして実行されることを前提にしているため、この設定がないと`aikyo`の各パッケージが正しく動作しません。</Aside>
...
TypeScriptファイルをコンパイルなしで実行するために、ランタイムとして[`tsx`](https://github.com/privatenumber/tsx)を開発依存に追加します。
...
```bash
$ pnpm exec tsx firehose.ts
$ pnpm exec tsx aya.ts
```
